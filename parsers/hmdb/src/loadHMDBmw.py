...
##############
# Class: HMDB metabolites loader (Edited by: Maydan Wienreb)
#
# By: Phil Owen
# Date: 3/31/2021
...
class HMDBLoader(SourceDataLoader):
                    ...
                    if metabolite_accession is not None and metabolite_accession.text is not None:
                        ...
                        if metabolite_name is not None and metabolite_name.text is not None:
                            ...
                            metabolite_node = kgxnode(
                                metabolite_id,
                                name=metabolite_name.text.encode('ascii',errors='ignore').decode(encoding="utf-8"),
    ...
    def get_node_properties(self, el) -> dict:
        ...
        def safe_process_text(text):
            if text:
                return text.encode('utf-8').decode('utf-8')  # Ensure proper UTF-8 encoding/decoding
            return ''
        ...
        # Extract synonyms
        synonyms = [
            safe_process_text(syn.text) for syn in el.find('synonyms').findall('synonym') if syn.text
        ]
        synonyms = [s for s in synonyms if s]
        if synonyms:
            node_properties['synonyms'] = synonyms

        # Extract chemical_formula
        chemical_formula = el.find('chemical_formula').text if el.find('chemical_formula') is not None else ''
        if chemical_formula:
